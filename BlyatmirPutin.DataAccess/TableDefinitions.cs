...
namespace BlyatmirPutin.DataAccess
{
	...
	public static class TableDefinitions
	{
		...
		public static readonly Dictionary<Type, string> Mappings = new Dictionary<Type, string>()
		{
			...
			{ typeof(IntroMusic), "CREATE TABLE IntroMusic (Id INT IDENTITY, UploaderId BIGINT NOT NULL, IntroName NVARCHAR NOT NULL, FilePath NVARCHAR NOT NULL, DateAdded DATETIME DEFAULT GETDATE(), PRIMARY KEY (Id));" },
			{ typeof(IntroMusicRecord), "CREATE TABLE IntroMusicModule (Id INT IDENTITY, GuildId BIGINT, IsEnabled BIT DEFAULT 1, PRIMARY KEY (Id));" },
